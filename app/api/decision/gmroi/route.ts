...
import { safeDivide } from "@/lib/data-quality";

const PROJECT = "project-cb954e13-3b16-432f-aa7.analytics_lab";
const STRONG_BENCHMARK = 3.2;
const ATTENTION_THRESHOLD = 2.0;
  ...
  try {
    ...
    const categories = Object.entries(byCategory).map(([category, v]) => {
      ...
      return {
        ...
        gmroi: gmroiRounded,
        status: typeof gmroiRounded !== "number"
          ? "unknown"
          : gmroiRounded >= STRONG_BENCHMARK
          ? "strong"
          : gmroiRounded < ATTENTION_THRESHOLD
          ? "attention"
          : "adequate",
      };
    ...
    return NextResponse.json({
      categories,
      overallGmroi,
      strongBenchmark: STRONG_BENCHMARK,
