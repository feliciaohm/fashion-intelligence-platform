// Shared real computation behind /api/growth-bridge, extracted so
// lib/ai-demo-mode.ts can answer "why did revenue change" questions from the
// exact same numbers the Growth Bridge waterfall shows. See
// app/growth-bridge's page for the UI that consumes /api/growth-bridge,
// which now just calls this.
import { bigquery } from "@/lib/bigquery";

const PROJECT = "project-cb954e13-3b16-432f-aa7.analytics_lab";

export interface GrowthBridgeSummary {
  ...
  waterfall: { label: string; value: number; isTotal?: boolean }[];
  methodology: string[];
}

export async function getGrowthBridgeSummary(): Promise<GrowthBridgeSummary> {
  // Same real-data substitution and equal-count cohort split established for
  // Revenue Growth Decomposition: this dataset has no real multi-year
  // history, so "last year vs this year" is built from the two real halves
  // of the purchase history instead of fabricating prior-year data. Applied
  // per-customer here (not per-category) since a growth bridge is a
  // customer-cohort analysis by definition.
  ...
  const methodology = [
    `Data source: shopify_orders and crm_customers alone can't build a real revenue bridge (crm_customers has no per-period revenue split) — real per-customer, per-purchase revenue and timestamps come from sales_events, joined to crm_customers only implicitly via the shared customer_id. This is the same real-data substitution already established for Revenue Growth Decomposition.`,
    ...
    `Price Effect = for the same retained customers, the change in their average order value (price/mix per order) applied to their current-period order count — isolates a real price movement from a real volume movement, same decomposition math as Revenue Growth Decomposition, applied per-customer instead of per-category.`,
